...
export default function WorkoutScreen({
  ...
  const [timeLeft, setTimeLeft] = useState(
    queue[0]?.exercise.duration_sec || 0,
  );
  ...
  const progress = currentExercise
    ? ((currentExercise.duration_sec - timeLeft) /
  ...
  const toggleTimer = () => {
    if (isActive) {
      ...
      if (pauseStartTimeRef.current) {
        setTotalPauseTime(
          (prev) => prev + (Date.now() - pauseStartTimeRef.current!),
        );
  ...
  const handleNext = () => {
    if (currentIndex < queue.length - 1) {
      ...
      setTimeLeft(queue[currentIndex + 1].exercise.duration_sec);
      setIsActive(true); // Auto-start next
    } else {
      finishWorkout();
    }
  };
      ...
      >
        ...
        <div>
          <h2 className="text-3xl font-bold mb-2">
            {t("app.workout_complete")}
          </h2>
          <p className="text-text-secondary">
            You completed {queue.length} exercises!
          </p>
        </div>
        ...
        <div className="w-full bg-surface rounded-2xl p-6 shadow-sm border border-border space-y-4">
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="font-bold text-lg">
              {Math.floor(totalPauseTime / 1000)}s
            </span>
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col h-full relative">
      <div className="flex justify-between items-center mb-6">
        <button
          onClick={onCancel}
          className="p-2 text-text-secondary hover:text-text-secondary"
        >
          <X size={24} />
        </button>
          ...
          >
            ...
            <div className="p-6 text-center">
              <h2 className="text-2xl font-bold mb-3">
                {t(`exercises.${currentExercise.name_key}`)}
              </h2>
              <p className="text-text-secondary text-sm mb-6">
                {t(`exercises.${currentExercise.description_key}`)}
              </p>

              <div className="flex justify-center gap-2 mb-8">
                <span className="text-xs px-3 py-1.5 bg-surface rounded-lg font-medium">
                  {currentExercise.difficulty}
                </span>
                <span className="text-xs px-3 py-1.5 bg-primary/10 text-primary rounded-lg font-medium">
                  {currentExercise.tools}
                </span>
              </div>
              ...
              <div className="relative w-48 h-48 mx-auto mb-8">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  ...
                  <span className="text-sm text-text-secondary uppercase tracking-widest mt-1">
                    {t("app.sec")}
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-center gap-6">
                <button
                  onClick={toggleTimer}
                  className="w-20 h-20 bg-primary text-white rounded-full flex items-center justify-center hover:bg-primary/90 transition-transform active:scale-95 shadow-lg "
                >
                  {isActive ? (
                ...
                <button
                  onClick={handleNext}
                  className="w-14 h-14 bg-surface text-text-secondary rounded-full flex items-center justify-center hover:bg-border  transition-transform active:scale-95"
                >
